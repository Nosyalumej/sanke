...
restart :: proc() {
    start_head_postition := Vec2i { GRID_WIDTH / 2, GRID_WIDTH / 2}
    snake[0] = start_head_postition
    snake[1] = start_head_postition - {0, 1}
    snake[2] = start_head_postition - {0, 2}
    snake_length = 3
    move_direction = {0, 1}
    game_over = false
    place_food()
...
main :: proc() {
    ...
    rl.InitWindow(WINDOW_SIZE, WINDOW_SIZE, "Sanke Game") 
    rl.InitAudioDevice()   
    ...
    for !rl.WindowShouldClose(){
        ...
        if rl.IsKeyDown(.UP) && old_direction.x != 0 {
        move_direction = {0, -1}
        }
        if rl.IsKeyDown(.DOWN) && old_direction.x != 0 {
        move_direction = {0, 1}
        }
        if rl.IsKeyDown(.LEFT) && old_direction.y != 0 {
        move_direction = {-1, 0}
        }
        if rl.IsKeyDown(.RIGHT) && old_direction.y != 0 {
        move_direction = {1, 0}
        }
        ...
        if tick_timer <= 0 {
            ...
            for i in 1..<snake_length {
                ...
                if current_part_position == head_pos {
                    game_over = true
                    rl.PlaySound(crash_sound)
                }
                snake[i] = next_part_postition
                next_part_postition = current_part_position
            }
            if head_pos == food_pos {
                snake_length += 1
                snake[snake_length - 1] = next_part_postition
                place_food()
        ...
        for i in 0..<snake_length {
            ...
            rl.DrawTexturePro(part_sprite, source, destination, {CELL_SIZE, CELL_SIZE}*0.5, rotation, rl.WHITE) 
        }
        if game_over {
            rl.DrawText("Game Over", 10, 10, 20, rl.WHITE)
            rl.DrawText("Press Enter to restart", 10, 30, 20, rl.WHITE)
        }
